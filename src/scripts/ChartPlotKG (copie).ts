...
function renderPie(id: string, data: DataPoint[], endAngle = 270) {
  console.log('rendu pie ' + id)
  const am = getAm()
  if (!am || !elExists(id)) return
  const { am5, am5percent, am5themes_Animated } = am

  disposeRootIfExists(id)
...
function renderBarCategory(id: string, data: DataPoint[]) {
  const am = getAm()
  if (!am || !elExists(id)) return
  const { am5, am5xy, am5themes_Animated } = am
  ...
  chart.set('cursor', am5xy.XYCursor.new(root, { lineY: { visible: false } }))
  ...
  series.columns.template.adapters.add('stroke', (_stroke: any, target: any) =>
    chart.get('colors').getIndex(series.columns.indexOf(target))
  )
